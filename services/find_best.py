...
prompt= """
Найди интригующие моменты в тексте (ВАЖНО: тема должна начинаться и заканчиваться в предложениях, которые ты вернул. Лучше захватить лишние, чем упустить важное). Максимум 3 момента, комбинировать чанки надо от 6 до 90 элементов. 

Моментами считаются только те случаи, где начало и конец составляет конечную мысль. Момент может закончится только на логической конструкции, которая завершает мысль или подводит итог!  

Не пытайся сильно сокращать моменты, в норме моменты должны быть примерно 30 чанков.

Интригующими считаются моменты, где происходит что-то неожиданное, раскрываются важные детали или возникают сильные эмоции.
...
prompt_for_sort= """
Отсортируй текст, так чтобы самыми первыми были те что заканчиваются на нормальные логические конструкции, если оба текста хорошо заканчиваются выбирай между ними по самому интересному, интригующему.
Ответ должен содержать массив с объектами id и description, description должен содержать краткое описание почему ты поставил их именно в таком порядке относительно других.
Формат входных данных <id>. [<texts>]
Текст:
%s 

Ответ:
json
{sortedTexts: [{"id": <id>, "description": <description>}]}
"""
